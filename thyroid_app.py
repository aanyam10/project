...
def main():
    ...
    with st.form(key='input_form'):
        # Feature: Age
        st.markdown("<p style='font-size: 16px;'>Age:</p>", unsafe_allow_html=True)
        age = st.number_input("", min_value=0, max_value=120, step=1)

        # Feature: Stage
        st.markdown("<p style='font-size: 16px;'>Stage:</p>", unsafe_allow_html=True)
        stage = st.selectbox("", options=[0, 1, 2, 3, 4], format_func=lambda x: {
            0: "I", 1: "II", 2: "III", 3: "IVB", 4: "IVA"
        }[x])

        # Feature: T (Tumor size)
        st.markdown("<p style='font-size: 16px;'>T (Tumor size):</p>", unsafe_allow_html=True)
        ...
        st.markdown("<p style='font-size: 16px;'>N (Node involvement):</p>", unsafe_allow_html=True)
        n = st.selectbox("", options=[0, 1, 2], format_func=lambda x: {
            0: "N0", 1: "N1b", 2: "N1a"
        }[x])

        # Feature: Adenopathy
        st.markdown("<p style='font-size: 16px;'>Adenopathy:</p>", unsafe_allow_html=True)
        ...
        # Feature: Response
        st.markdown("<p style='font-size: 16px;'>Response:</p>", unsafe_allow_html=True)
